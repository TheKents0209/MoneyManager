    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        LazyColumn(modifier = Modifier
            ...
            groups.forEach { groupString ->
                item {
                    Text(groupString, color = MaterialTheme.colors.onBackground, fontWeight = FontWeight.Bold, fontSize = 20.sp)
                    Column {
                        list?.forEach {
                            if(groupString == it.group) {
                                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                                    Text(text = it.name + "  " + intToCurrencyString(aViewModel.getAccountAmount(it.id).observeAsState().value), modifier = Modifier
                                    ...
                                    if(isModifyEnabled) {
                                        IconButton(onClick = {
                                            ...
                                            Icon(
                                                painterResource(R.drawable.ic_twotone_remove_circle_outline_24), contentDescription = "Delete account", tint = Color.Red
                                            )
                                        }
                                    }
                                }
